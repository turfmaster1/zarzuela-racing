...
function updateCamera(dt){if(!runners.length)return;const s=sorted(),lead=s[0],center=new THREE.Vector3();s.forEach(r=>center.add(r.root.position));center.divideScalar(s.length);const f=THREE.MathUtils.clamp(lead.distance/race.distance,0,1),tan=route.getTangentAtFraction(f),side=new THREE.Vector3(tan.z,0,-tan.x).normalize(),remaining=race.distance-lead.distance;let desired,target=center.clone();if(state.cameraMode===0){const final=remaining<500;desired=center.clone().addScaledVector(side,final?34:54).addScaledVector(tan,final?-16:-28);desired.y=final?9:19;target.y=final?1.8:3.1;}else if(state.cameraMode===1){desired=center.clone();desired.y=72;target.y=0;}else if(state.cameraMode===2){desired=center.clone().addScaledVector(side,20).addScaledVector(tan,-5);desired.y=5.5;target.addScaledVector(tan,11);target.y=1.8;}else{desired=new THREE.Vector3(FINISH_X+28,8,BOTTOM_Z+24);target=new THREE.Vector3(FINISH_X,1.7,BOTTOM_Z);}camera.position.lerp(desired,1-Math.exp(-4*dt));camera.lookAt(target);}

...
function createFinishHorseshoe(){const group=new THREE.Group(),black=new THREE.MeshLambertMaterial({color:0x111214}),white=new THREE.MeshLambertMaterial({color:0xffffff}),green=new THREE.MeshLambertMaterial({color:0x0b6c50}),fx=FINISH_X+.10,fz=RAIL_Z_INNER-.82;const pts=[[-4.7,.1],[-4.7,2.1],[-4.2,3.55],[-3.1,4.85],[-1.55,5.7],[0,6],[1.55,5.7],[3.1,4.85],[4.2,3.55],[4.7,2.1],[4.7,.1]].map(([x,y])=>new THREE.Vector3(fx+x,y,fz));group.add(new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts,false,'centripetal'),44,.58,7,false),black));const inner=[[-3.75,.75],[-3.72,2.3],[-3.15,3.52],[-2.05,4.48],[-.85,4.98],[0,5.08],[.85,4.98],[2.05,4.48],[3.15,3.52],[3.72,2.3],[3.75,.75]].map(([x,y])=>new THREE.Vector3(fx+x,y,fz-.42));group.add(new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(inner,false,'centripetal'),34,.045,4,false),white));const logo=new THREE.Mesh(new THREE.PlaneGeometry(3.15,1.10),new THREE.MeshBasicMaterial({map:finishTextTexture(),transparent:true,side:THREE.DoubleSide,depthWrite:false}));logo.position.set(fx,5.43,fz-.68);group.add(logo);const lp=new THREE.Mesh(new THREE.BoxGeometry(.1,2.35,.1),green);lp.position.set(fx-.27,1.18,fz+.12);group.add(lp);const rp=lp.clone();rp.position.x=fx+.27;group.add(rp);const top=new THREE.Mesh(new THREE.BoxGeometry(.66,.1,.1),green);top.position.set(fx,2.3,fz+.12);group.add(top);const base=new THREE.Mesh(new THREE.BoxGeometry(3.05,.82,.92),black);base.position.set(fx,.41,fz-.02);group.add(base);scene.add(group);const finishLine=new THREE.Mesh(new THREE.BoxGeometry(.55,.025,TRACK_WIDTH),new THREE.MeshBasicMaterial({color:0xffffff}));finishLine.position.set(FINISH_X,.03,BOTTOM_Z);scene.add(finishLine);}

function createGrandstand(){const g=new THREE.Group();g.position.set(60,0,BOTTOM_Z+72);const concrete=new THREE.MeshLambertMaterial({color:0xdad8d0}),dark=new THREE.MeshLambertMaterial({color:0x4b504d}),seat=new THREE.MeshLambertMaterial({color:0x65716b});for(let m=-1;m<=1;m++){const bx=m*76;for(let i=0;i<8;i++){const step=new THREE.Mesh(new THREE.BoxGeometry(70,1.1,4.6),seat);step.position.set(bx,.55+i*.82,i*3.8);g.add(step);}const back=new THREE.Mesh(new THREE.BoxGeometry(72,15,2),concrete);back.position.set(bx,8,31);g.add(back);const roof=new THREE.Mesh(new THREE.BoxGeometry(76,.7,25),concrete);roof.position.set(bx,18,22);roof.rotation.x=.12;g.add(roof);for(let c=-2;c<=2;c++){const support=new THREE.Mesh(new THREE.CylinderGeometry(.28,.4,16,10),dark);support.position.set(bx+c*14,9,30);support.rotation.z=.12;g.add(support);}}scene.add(g);}

function buildWorld(){if(worldBuilt)return;worldBuilt=true;const dirt=createDirtTexture();dirt.repeat.set(45,32);const ground=new THREE.Mesh(new THREE.PlaneGeometry(2600,1800),new THREE.MeshLambertMaterial({map:dirt}));ground.rotation.x=-Math.PI/2;ground.position.y=-.05;scene.add(ground);const grass=createGrassTexture();grass.repeat.set(52,4);const trackMat=new THREE.MeshLambertMaterial({map:grass,color:0xffffff,side:THREE.DoubleSide});const straight=new THREE.Mesh(new THREE.PlaneGeometry(TRACK_END_EXTENSION-TRACK_START_EXTENSION,TRACK_WIDTH),trackMat);straight.rotation.x=-Math.PI/2;straight.position.set((TRACK_START_EXTENSION+TRACK_END_EXTENSION)/2,0,BOTTOM_Z);scene.add(straight);scene.add(createTrackRibbon(mainCourseCurve,TRACK_WIDTH,trackMat));const railMat=new THREE.MeshLambertMaterial({color:0xf4f4f1});createStraightRailSection(TRACK_START_EXTENSION,TRACK_END_EXTENSION,RAIL_Z_OUTER,railMat);createStraightRailSection(TRACK_START_EXTENSION,PARDO_JOIN_X-48,RAIL_Z_INNER,railMat);createStraightRailSection(PARDO_JOIN_X+20,TRACK_END_EXTENSION,RAIL_Z_INNER,railMat);createOffsetRailCurve(RAIL_OFFSET,.002,Math.min(1,PARDO_JOIN_U+.012),railMat);createOffsetRailCurve(-RAIL_OFFSET,.002,Math.max(.002,PARDO_JOIN_U-.05),railMat);for(const d of [600,500,400,300,200,100])createDistanceMarker(FINISH_X-d,String(d));createFinishHorseshoe();createGrandstand();}
...
function buildLongRoute(d){const nominalLap=1600,loops=Math.ceil(d/nominalLap),startNominal=loops*nominalLap-d,startIndex=Math.floor((startNominal/nominalLap)*(CLOSED_LAP.length-1)),pts=[];for(let i=startIndex;i<CLOSED_LAP.length;i++)pts.push(CLOSED_LAP[i].clone());for(let l=1;l<loops;l++)for(let i=1;i<CLOSED_LAP.length;i++)pts.push(CLOSED_LAP[i].clone());return new RaceRoute(pts);}
...
function makeRunner(h){
  ...
  model.traverse(o=>{
    ...
    for(const mat of mats){
      ...
      if(name.includes('horse.body.pattern'))setMaterialColor(mat,h.coat);
...
function animateGates(dt){if(!gateGroup?.userData.opening)return;gateGroup.userData.t=Math.min(1,gateGroup.userData.t+dt*4);const e=1-Math.pow(1-gateGroup.userData.t,3);gateGroup.userData.doors.forEach(d=>d.pivot.rotation.y=d.side*e*1.25);}
...
function updateCamera(dt){if(!runners.length)return;const s=sorted(),lead=s[0],center=new THREE.Vector3();s.forEach(r=>center.add(r.root.position));center.divideScalar(s.length);const f=THREE.MathUtils.clamp(lead.distance/race.distance,0,1),tan=route.getTangentAtFraction(f),side=new THREE.Vector3(tan.z,0,-tan.x).normalize(),remaining=race.distance-lead.distance;let desired,target=center.clone();if(state.cameraMode===0){const final=remaining<500;desired=center.clone().addScaledVector(side,final?30:44).addScaledVector(tan,final?-13:-22);desired.y=final?8:15;target.y=final?1.8:2.7;}else if(state.cameraMode===1){desired=center.clone();desired.y=72;target.y=0;}else if(state.cameraMode===2){desired=center.clone().addScaledVector(side,18).addScaledVector(tan,-5);desired.y=5.2;target.addScaledVector(tan,10);target.y=1.8;}else if(state.cameraMode===3){desired=lead.root.position.clone().addScaledVector(side,10).addScaledVector(tan,-6);desired.y=3.4;target=lead.root.position.clone().addScaledVector(tan,3.5);target.y=1.65;}else{desired=new THREE.Vector3(FINISH_X+28,8,BOTTOM_Z+24);target=new THREE.Vector3(FINISH_X,1.7,BOTTOM_Z);}camera.position.lerp(desired,1-Math.exp(-4.8*dt));camera.lookAt(target);}
